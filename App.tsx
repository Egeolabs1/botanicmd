...
const AppContent = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && appState === AppState.LANDING) {
  ...
  const handleLogout = async () => {
    ...
    // Reset all app-specific states for a clean session
    setPlantData(null);
    setImagePreview(null);
    setError(null);
    // The saved plants are tied to the user, so they should be cleared on logout.
    // The next login will refetch them.
    setSavedPlants([]); 
    setCandidates([]);
  ...
  if (appState === AppState.BLOG) {
    return (
      <>
         <BlogPage onBack={() => setAppState(isAuthenticated ? AppState.IDLE : AppState.LANDING)} />
         <PWAInstallPrompt />
      </>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-gray-100/80 p-1.5 rounded-full border border-gray-200/50 absolute left-1/2 transform -translate-x-1/2">
             ...
             >
              ...
              <button
               onClick={() => setAppState(AppState.BLOG)}
               className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 ${
